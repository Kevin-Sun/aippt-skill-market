...
### 0. ci-upload 版本号铁律（2026-08-22 事故）
- `node scripts/ci-upload.js --ver 1.5.4 --desc "版本描述"`（也接受裸位置参数）
- **上传后必须核对输出 `Version: x.y.z` 与预期一致**——2026-08-22 曾因位置参数不识别静默回落 1.0.0，mp 后台版本号错标
- 脚本已加强校验：版本号缺失/非法直接 exit 1
- `--desc` 必须写有意义的版本描述（审核语境、面向用户的修复说明），禁止时间戳占位
